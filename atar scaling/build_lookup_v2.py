...
# ============================================================
# 1. Reference data: Two years of aggregate-to-ATAR mappings
# ============================================================

# Previous year (likely 2024) data
prev_year_data = [
...
year_2023_data = [
    ...
    (480.68, 99.82), (480.59, 98.80), (480.43, 98.77), (480.43, 98.77),
...
def clean_and_average(data):
    """Sort by ATAR, average duplicate ATARs, return unique arrays."""
...
print(f"Previous year: {len(prev_atars)} unique ATAR points, range {prev_atars[0]:.2f}-{prev_atars[-1]:.2f}")
print(f"  Aggregate range: {prev_aggs[0]:.2f}-{prev_aggs[-1]:.2f}")
print(f"2023 year:     {len(y23_atars)} unique ATAR points, range {y23_atars[0]:.2f}-{y23_atars[-1]:.2f}")
print(f"  Aggregate range: {y23_aggs[0]:.2f}-{y23_aggs[-1]:.2f}")

# Build PCHIP interpolators for each year: ATAR -> Aggregate
interp_prev = PchipInterpolator(prev_atars, prev_aggs)
interp_2023 = PchipInterpolator(y23_atars, y23_aggs)

# Compare at key points
print(f"\n{'ATAR':>8} | {'Prev Agg':>10} | {'2023 Agg':>10} | {'Diff':>8}")
print("-" * 45)
for a in [99.95, 99.00, 98.00, 97.00, 95.00, 93.00, 90.00, 85.00, 80.00,
          75.00, 70.00, 65.00, 60.00, 55.00, 50.00, 45.00, 40.00]:
    if a >= prev_atars[0] and a <= prev_atars[-1] and a >= y23_atars[0] and a <= y23_atars[-1]:
        p = float(interp_prev(a))
        y = float(interp_2023(a))
        print(f"{a:8.2f} | {p:10.2f} | {y:10.2f} | {y-p:+8.2f}")

# ============================================================
# 2. Build averaged/blended model from both years
# ============================================================
# Create a blended model that averages both years' curves
# This gives a more robust estimate of the aggregate-ATAR relationship

# Common ATAR range for blending
common_min = max(prev_atars[0], y23_atars[0])
common_max = min(prev_atars[-1], y23_atars[-1])

# For the blended model, weight more recent year slightly more (60/40)
WEIGHT_PREV = 0.60  # More recent year gets higher weight
WEIGHT_2023 = 0.40

# Evaluate both on a fine grid and blend
fine_atars = np.arange(common_min, common_max + 0.01, 0.05)
fine_atars = np.round(fine_atars, 2)
blended_aggs = WEIGHT_PREV * interp_prev(fine_atars) + WEIGHT_2023 * interp_2023(fine_atars)

# Build blended interpolator
interp_blended = PchipInterpolator(fine_atars, blended_aggs)

# Extend below common range using 2023 data (which goes lower)
# and above using extrapolation
def get_aggregate(atar):
    """Get aggregate for a given ATAR using blended model with extensions."""
    if atar >= common_min and atar <= common_max:
        return float(interp_blended(atar))
    elif atar < common_min:
        # Below common range: use 2023 curve (it extends lower)
        if atar >= y23_atars[0]:
            return float(interp_2023(atar))
        else:
            # Extrapolate below 2023 range
            slope = float(interp_2023.derivative()(y23_atars[0]))
            return float(y23_aggs[0]) + slope * (atar - y23_atars[0])
    else:
        # Above common range: extrapolate
        slope = float(interp_blended.derivative()(common_max))
        val = float(blended_aggs[-1]) + slope * (atar - common_max)
        return min(val, 500.0)

# ============================================================
# 3. 2025 ATAR distribution from report
# ============================================================
table13 = [
...
# ============================================================
# 4. Build 0.05-band distribution
# ============================================================
band_students = {}
...
# ============================================================
# 5. Build cumulative and generate lookup
# ============================================================
...
print(f"\nTotal students in bands 30.05-99.95: {running_total} (expected 29,869)")

# Generate results
results = []
for band in sorted_bands:
    atar = band
    agg = get_aggregate(atar)
    agg = max(0.0, min(500.0, agg))
...
# ============================================================
print(f"\n{'='*75}")
print(f"  2025 QCE AGGREGATE TO ATAR LOOKUP TABLE (Blended 60/40 model)")
print(f"  Based on 2 years of reference data, weighted 60% recent / 40% 2023")
print(f"{'='*75}")
print(f"\n{'ATAR':>8} | {'Aggregate':>10} | {'Students':>8} | {'Cumulative':>10} | {'Cum %':>8}")
print("-" * 55)

# Print every band from 99.95 to 98.00, then every 0.50 to 90, then every 1.00 below
for row in results:
    atar = row[0]
    show = False
    if atar >= 98.00:
        show = True
    elif atar >= 90.00 and (atar * 100) % 50 < 1:
        show = True
    elif atar < 90.00 and (atar * 100) % 100 < 1:
        show = True
    if show:
...
# ============================================================
# Full detailed table
csv_path = "C:/PSAM/QLD/atar scaling/aggregate_to_atar_2025_v2.csv"
with open(csv_path, 'w', newline='') as f:
    writer = csv.writer(f)
    writer.writerow(['ATAR', 'Aggregate', 'Students_in_Band', 'Cumulative_Students', 'Cumulative_Pct'])
    for row in results:
        writer.writerow(row)
print(f"\nFull table saved to: {csv_path} ({len(results)} rows)")

# Simplified Aggregate -> ATAR lookup (sorted by aggregate descending)
csv_path2 = "C:/PSAM/QLD/atar scaling/aggregate_atar_lookup_2025_v2.csv"
...
# ============================================================
# 8. Cross-validation: show how the blended model compares
# ============================================================
print(f"\n{'='*65}")
print(f"  CROSS-VALIDATION: Blended model vs each reference year")
print(f"{'='*65}")
print(f"{'ATAR':>8} | {'Blended':>10} | {'Prev Yr':>10} | {'2023':>10} | {'Bl-Prev':>8} | {'Bl-2023':>8}")
print("-" * 65)
for a in [99.95, 99.50, 99.00, 98.00, 97.00, 96.00, 95.00, 94.00, 93.00,
          92.00, 91.00, 90.00, 88.00, 86.00, 84.00, 82.00, 80.00, 78.00,
          76.00, 74.00, 72.00, 70.00, 65.00, 60.00, 55.00, 50.00, 45.00, 40.00]:
    bl = get_aggregate(a)
    pv = float(interp_prev(a)) if a >= prev_atars[0] and a <= prev_atars[-1] else None
    y2 = float(interp_2023(a)) if a >= y23_atars[0] and a <= y23_atars[-1] else None
    pv_str = f"{pv:10.2f}" if pv else "       N/A"
    y2_str = f"{y2:10.2f}" if y2 else "       N/A"
    dp = f"{bl-pv:+8.2f}" if pv else "     N/A"
    dy = f"{bl-y2:+8.2f}" if y2 else "     N/A"
    print(f"{a:8.2f} | {bl:10.2f} | {pv_str} | {y2_str} | {dp} | {dy}")

# ============================================================
# 9. Also add bands below 30 for the 298 unreported students
# ============================================================
# Extrapolate below 30.05 to capture students with ATAR <= 30
# We'll add bands from 30.00 down to approximately 13.00
# (report says 298 students had ATA of 13 or below... but
#  actually the 298 are "30 or below", not "13 or below")
# The ATAR band constraint at these low levels gives very few per band
print(f"\n{'='*65}")
print(f"  EXTENDED TABLE: Below ATAR 30 (298 students)")
print(f"{'='*65}")

# From Table 14, the last group (30.05-30.95) has 43 students in 19 bands
# That's about 2.3 per band. Below 30, it would be even fewer.
# We need to distribute 298 students across bands from ~13 to 30.00
# The participation model curve continues to decrease

# Estimate: from the pattern, bands below 30 get roughly 1-2 students each
# 298 students across bands from ~13 to 30 in 0.05 steps = 340 bands
# Average ~0.88 per band. Some bands may be empty.

# For the lookup table, what matters is the aggregate cutoff for ATAR 30.00
agg_30 = get_aggregate(30.05)
print(f"Aggregate at ATAR 30.05: {agg_30:.2f}")
print(f"298 students below ATAR 30 are not individually banded in the QTAC report")
print(f"For practical purposes, any aggregate below ~{agg_30:.0f} would receive ATAR <= 30")
