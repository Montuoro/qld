...
def clean_and_average(data):
    """Sort by ATAR, average duplicate ATARs, return unique sorted arrays."""
...
# ============================================================
# STRATEGY: Pool both years into a single dataset
# ============================================================
# Rather than blending two separate interpolations (which causes wiggles),
# we combine all data points, giving more recent year slightly more weight
# by duplicating its points.

# Combine: include prev year points once, 2023 points once
# (equal weight - both are valid reference years)
all_atars = np.concatenate([prev_atars, y23_atars])
all_aggs = np.concatenate([prev_aggs, y23_aggs])

# Sort by ATAR and average duplicates/near-duplicates
# Use 0.05 bins to smooth
bin_edges = np.arange(30.00, 100.025, 0.05)
bin_atars = []
bin_aggs = []
for i in range(len(bin_edges) - 1):
    ...
    mask = (all_atars >= lo) & (all_atars < hi)
    if np.sum(mask) > 0:
        bin_atars.append(center)
        bin_aggs.append(np.mean(all_aggs[mask]))

bin_atars = np.array(bin_atars)
bin_aggs = np.array(bin_aggs)

# Enforce strict monotonicity (higher ATAR = higher aggregate)
# Pass forward: if agg[i] <= agg[i-1], set agg[i] = agg[i-1] + 0.01
for i in range(1, len(bin_aggs)):
    if bin_aggs[i] <= bin_aggs[i - 1]:
        bin_aggs[i] = bin_aggs[i - 1] + 0.01

# Build the final PCHIP interpolator on the cleaned, monotonic data
interp_final = PchipInterpolator(bin_atars, bin_aggs)

# ============================================================
# 2025 ATAR distribution
# ============================================================
table13 = [
...
# ============================================================
# Generate aggregate for each ATAR band
# ============================================================
raw_results = []
for band in sorted_bands:
    atar = band
    if atar >= bin_atars[0] and atar <= bin_atars[-1]:
        agg = float(interp_final(atar))
    elif atar < bin_atars[0]:
        # Linear extrapolation below
        slope = float(interp_final.derivative()(bin_atars[0]))
        agg = float(bin_aggs[0]) + slope * (atar - bin_atars[0])
    else:
        slope = float(interp_final.derivative()(bin_atars[-1]))
        agg = float(bin_aggs[-1]) + slope * (atar - bin_atars[-1])
    agg = max(0.0, min(500.0, agg))
    raw_results.append((atar, agg, band_students[band], cumulative[band]))

# ============================================================
# ENFORCE STRICT MONOTONICITY on final results
# ============================================================
# Higher ATAR must have higher aggregate. Process top-down.
results = []
for i, (atar, agg, n_stu, cum_stu) in enumerate(raw_results):
    if i > 0 and agg >= results[-1][1]:
        # Current agg >= previous (higher ATAR) - violation
        agg = results[-1][1] - 0.01
    results.append((atar, round(agg, 2), n_stu, cum_stu, round(cum_stu / TOTAL_STUDENTS * 100, 2)))
...
print(f"  Model: Pooled 2-year reference data with monotonicity enforcement")
print(f"  Total ATAR-eligible students: {TOTAL_STUDENTS:,}")
print(f"  Students below ATAR 30: {BELOW_30_STUDENTS}")
print(f"{'='*80}")
...
for row in results:
    ...
    if atar >= 98.00:
        show = True
    elif atar >= 90.00 and abs(atar * 20 - round(atar * 20)) < 0.01:
        show = True  # every 0.05 (all of them, but let's do every 0.50)
    if atar >= 90 and (atar * 100) % 50 < 1:
        show = True
...
# Save CSV
# ============================================================
csv_path = "C:/PSAM/QLD/atar scaling/aggregate_to_atar_2025_final.csv"
with open(csv_path, 'w', newline='') as f:
    writer = csv.writer(f)
    writer.writerow(['ATAR', 'Aggregate', 'Students_in_Band', 'Cumulative_Students', 'Cumulative_Pct'])
    for row in results:
        writer.writerow(row)
print(f"\nFull table: {csv_path} ({len(results)} rows)")

# Simplified Aggregate -> ATAR lookup
csv_path2 = "C:/PSAM/QLD/atar scaling/aggregate_atar_lookup_2025_final.csv"
with open(csv_path2, 'w', newline='') as f:
    writer = csv.writer(f)
    writer.writerow(['Aggregate', 'ATAR'])
    for row in sorted(results, key=lambda x: x[1], reverse=True):
        writer.writerow([row[1], row[0]])
print(f"Lookup table: {csv_path2}")

# ============================================================
# Spot check against both reference years
# ============================================================
...
print(f"\n{'='*75}")
print(f"  SPOT CHECK vs Reference Years")
print(f"{'='*75}")
...
for a in [99.95, 99.00, 98.00, 97.00, 96.00, 95.00, 94.00, 93.00, 92.00,
          91.00, 90.00, 89.00, 88.00, 87.00, 86.00, 85.00, 84.00, 83.00,
          82.00, 81.00, 80.00, 78.00, 76.00, 74.00, 72.00, 70.00,
          65.00, 60.00, 55.00, 50.00, 45.00, 40.00, 35.00, 31.00]:
